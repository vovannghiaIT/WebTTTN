...
const HomeOpera = () => {
  ...
  const currentItems = operas.slice(itemOffset, endOffset);
  const total = operas;
  ...
  const submitDelete = async () => {
    ...
    for (let i = 0; i <= check.length; i++) {
      // console.log(check[i]);
      let listImg = operas.filter((items) => items?._id === check[i]);

      // console.log(listImg);
      if (listImg[0]) {
        await apiDeleteOpera(listImg[0]);
      }
    }
  ...
  return (
    <div className="p-5 grid gap-4">
      <div className="flex justify-between items-center">
        <h1 className="capitalize text-xl font-semibold">Opera</h1>
        <div className="flex gap-2 items-center">
          <Link
            to="/admin/opera/insert"
