...
const Edit = () => {
  ...
  const fetchDataVocher = async () => {
    let _id = param?._id;
    let dataProduct = vouchers?.filter((items) => items?._id === _id);
  ...
  const handlesubmitSave = async () => {
    ...
    if (invalids === 0) {
      await apiUpdateVouchers(payload);
      fetchData();
      setImagesPreview([]);
      setPayload({
        code: "",
        ...
        <div className="p-2">
          ...
          <div className="grid grid-cols-3 gap-2 items-center w-full mt-2">
            ...
            <p className="text-left">
              <input
                type="number"
                className="border col-span-2 mx-2 py-1 px-4"
                value={payload?.price}
                onFocus={() => setInvalidFields([])}
                onChange={(e) =>
                  setPayload((prev) => ({
